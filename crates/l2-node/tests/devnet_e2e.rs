...
fn should_run() -> bool {
    std::env::var("DEVNET_E2E").map(|v| v == "1").unwrap_or(false)
...
async fn devnet_e2e_submit_txs_and_check_batch() {
    ...
    let node_handle = tokio::spawn(async move {
        ...
        let status = Command::new("cargo")
            .args([
                "run", "-p", "l2-node", "--",
                "--l2-db-path", db_path.path().to_str().unwrap(),
                "--l2-listen-addr", &format!("127.0.0.1:{port}"),
                "--ippan-rpc-url", &ippan_rpc_url,
                "--l2-leader", "true",
    ...
    for i in 0u64..20 {
        ...
        assert!(result["accepted"].as_bool().unwrap_or(false), "tx {i} not accepted: {result}");
    ...
    assert!(batcher["enabled"].as_bool().unwrap_or(false), "batcher not enabled");
    ...
    eprintln!("Posting stats: pending={}, posted={}, confirmed={}, failed={}",
...
async fn devnet_e2e_status_endpoint() {
    ...
    let resp = client
        .get(format!("{ippan_rpc_url}/status"))
        .send()
        .await;
    ...
    match resp {
        Ok(r) => {
            ...
            if r.status().is_success() {
                ...
                eprintln!("IPPAN RPC response: {}", serde_json::to_string_pretty(&body).unwrap_or_default());
            }
        }
        Err(e) => {
